...
<div class="absolute right-4 lg:right-8 bottom-8 z-20 flex flex-col items-end gap-3">
  {#if layout.isMobile}
    <Button
      variant="secondary"
      size="icon-lg"
      ...
      class={cn(
        "shadow-neu-depth! rounded-3xl corner-squircle transition-all duration-200 size-16",
        newEventOverlayState.open
          ? "pointer-events-none scale-0 opacity-0"
          : "scale-100 opacity-100",
      )}
    >
      <HugeiconsIcon icon={PlusSignIcon} size={24} strokeWidth={2} className="duotone-fill size-7" />
    </Button>
  {:else}
    <Button
      variant="default"
