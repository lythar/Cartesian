<script lang="ts">
	...
	import { mapInteractionState, type MapEventProperties, newEventOverlayState } from "./map-state.svelte";
...
<div class="w-[300px] overflow-hidden bg-background rounded-2xl">
    <div class="h-32 w-full bg-muted flex items-center justify-center">
        {#if images.length > 0}
            <div class="h-32 w-full bg-cover bg-center" style="background-image: url({getMediaUrl(images[0].id)})"></div>
        {:else}
    ...
    <div class="p-4">
        <div class="mb-2 flex flex-wrap gap-1">
            {#each displayTags as tag}
                <span class="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-medium text-secondary-foreground">
                    {tag}
                </span>
            {/each}
            {#if remainingTags > 0}
        ...
        <h3 class="mb-1 text-lg font-semibold leading-tight">{event.eventName}</h3>
        ...
        <div class="flex items-center justify-between">
             <div class="flex items-center gap-2">
                <Avatar class="h-6 w-6">
                    ...
                    <AvatarFallback>{getInitials((event.communityId ? event.communityName : event.authorName) || "")}</AvatarFallback>
                </Avatar>
