<script lang="ts">
	...
	import {
		createReverseGeocodeQuery,
		type ReverseGeocode,
	} from "$lib/api/queries/reverse-geocode.query";
	...
	const handleUpdateNewEventMarker = (e: mapboxgl.MapMouseEvent) => {
		...
		if (
			prev &&
			Math.abs(prev.lng - lng) < ANIMATION_LOCATION_DELTA &&
			Math.abs(prev.lat - lat) < ANIMATION_LOCATION_DELTA
		) {
	...
	$effect(() => {
		...
		map.on("click", handleUpdateNewEventMarker);
...
<div bind:this={markerWrapperElement} class="h-8 w-8">
	<img
...
<div class="absolute bottom-4 left-1/2 z-10 -translate-x-1/2 transform">
	{#if selectedLocation}
		<div
			...
			class="inline-flex min-w-50 flex-row justify-between gap-6 rounded-lg bg-background p-4 shadow-lg"
		>
			<div class="flex flex-col">
				{#if geocodeQuery?.isPending}
					<p
						class="animate-gradient bg-linear-to-r from-primary/40 via-primary to-primary/40 bg-size-[200%_100%] bg-clip-text text-sm font-semibold text-transparent"
					>
						Loading address...
					</p>
				...
				<p class="text-sm text-muted-foreground">
					{selectedLocation.lng.toFixed(2)}, {selectedLocation.lat.toFixed(2)}
				</p>
			</div>
			<!-- CTA -->
