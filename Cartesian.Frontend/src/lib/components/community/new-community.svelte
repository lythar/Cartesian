<script lang="ts">
	import {
		createPostCommunityApiCreate,
		type PostCreateCommunityBody,
				...
				<div class="space-y-3">
					...
					<Form.Field {form} name="inviteOnly" class="space-y-1.5">
						<Form.Control>
							{#snippet children({ props })}
								<div
									class="flex items-center justify-between rounded-xl border border-border/50 bg-muted/30 p-4 transition-all hover:bg-muted/50"
								>
									<div class="flex items-start gap-3">
										<div
											class="flex h-9 w-9 items-center justify-center rounded-lg bg-background shadow-sm ring-1 ring-black/5"
										>
									...
									<button
										...
										onclick={() =>
											($formData.inviteOnly = !$formData.inviteOnly)}
										class={cn(
			...
			<div class="flex items-center justify-end gap-3">
				...
				>
					{#if createCommunityMutation.isPending}
						<span
							class="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-background/30 border-t-background"
						></span>
						Creating...
					{:else}
						<HugeiconsIcon
